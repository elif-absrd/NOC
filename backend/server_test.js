...
// Serve static HLS files from media/live
app.use('/live', express.static(path.join(__dirname, 'media', 'live')));
// Serve the frontend directory
app.use(express.static(path.join(__dirname, '..', 'frontend')));
console.log('Serving frontend from:', path.join(__dirname, '..', 'frontend')); // Debug path
console.log('Serving HLS files from:', path.join(__dirname, 'media', 'live')); // Debug path
...
const config = {
    ...
    http: { port: 8001, allow_origin: '*', mediaroot: path.join(__dirname, 'media') },
    trans: {
...
const getStreamMetrics = (streamId, streamPath) => {
    return new Promise((resolve, reject) => {
        ...
        const checkHls = (attempt = 0) => {
            if (!fs.existsSync(hlsFilePath) && attempt < 10) {
                console.warn(`HLS file not found at ${hlsFilePath}, retrying (${attempt + 1}/10)...`);
                return setTimeout(() => checkHls(attempt + 1), 2000);
            }
            ...
            exec(ffprobeCommand, (error, stdout, stderr) => {
                ...
                if (error) {
                    ...
                    try {
                        ...
                        if (videoStream) {
                            ...
                            fps = den ? num / den : 24;
                            bitrate = parseInt(probeData.format.bit_rate) || 500000;
                ...
                const ffmpegCommand = `${config.trans.ffmpeg} -i ${hlsUrl} -f null - -vstats -loglevel debug -timeout 10000000`;
                exec(ffmpegCommand, { maxBuffer: 1024 * 1024 * 10 }, (err, out, stderr) => {
                    ...
                    if (err) {
                        ...
                        bandwidth = bitrate * (throughput || 1) / 8 || 0;
                    }
...
nms.on('postPublish', async (id, streamPath, args) => {
    ...
    setTimeout(async () => {
        try {
            ...
            console.error('Elasticsearch indexing error (start):', {
...
nms.on('donePublish', async (id, streamPath, args) => {
    ...
    try {
        ...
        console.error('Elasticsearch indexing error (end):', {
            message: error.message,
            name: error.name,
            meta: error.meta,
            body: error.meta?.body,
...
nms.on('preConnect', (id, args) => {
    console.log('HTTP Server Port:', config.http.port);
    console.log('NMS HTTP Config:', nms.getConfig().http);
});
...
app.listen(8000, () => {
    ...
    testElasticsearchIndexing(); // Test indexing on server start
});
...
async function testElasticsearchIndexing() {
    try {
        ...
        console.log('Test document indexed successfully:', response);
    } catch (error) {
        console.error('Test indexing error:', {
            message: error.message,
            name: error.name,
            meta: error.meta,
            body: error.meta?.body,
